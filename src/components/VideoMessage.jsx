...
import { Video, Trash2, CornerUpRight, RotateCcw, Reply, MoreVertical } from 'lucide-react';

/**
 * Component hiển thị tin nhắn video trong ứng dụng chat
 */
const VideoMessage = React.memo(({ message, isSentByUser, onDelete, onForward, onRevoke, onReply, isGroupChat, senderName }) => {
    ...
    const renderContent = () => {
        if (message.revoked) {
            return (
                <div className="bg-gray-200 w-64 h-36 flex items-center justify-center">
                    ...
                    {hasError && (
                        <div className="w-64 h-36 bg-gray-200 flex items-center justify-center">
                            <div className="text-center">
                                ...
                                <span className="text-gray-500">Không thể tải video</span>
                            </div>
                        </div>
                    )}
                    <video
                        src={videoUrl}
                        controls
                        className={`w-64 rounded-lg ${!isLoaded ? 'hidden' : ''}`}
